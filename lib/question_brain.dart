...
class QuestionBrain extends ChangeNotifier {
  ...
  final List<QuestionModel> _questions = [
    QuestionModel(const AssetImage('images/image1.png'), '考えるよりも先に行動するタイプだ。'),
    QuestionModel(
        const AssetImage('images/image2.png'), 'ショッピングに出かける頻度が多いと思う。'),
    QuestionModel(const AssetImage('images/image3.png'), '絶対に損をしたくない。'),
    QuestionModel(
        const AssetImage('images/image4.png'), '良い出来事が起きたときに気持ちが高揚しやすい。'),
    QuestionModel(const AssetImage('images/image5.png'), '買い物した後はしばらく満足してる。'),
    QuestionModel(const AssetImage('images/image6.png'),
        'つい目の前の作業を忘れ、他のことを考えてしまうことがよくある。'),
  ];

  ///TODO 診断結果の画像ができたらファイル名をresultにする
  final List<ResultModel> _results = [
    ResultModel(const AssetImage('images/result_a.png'), 'A. 衝動買いタイプ',
        'いいものを見つけるとテンションが上がるあなたは衝動買いタイプ。手を伸ばす前に深呼吸し本当に必要かよく考えてみて。'),
    ResultModel(const AssetImage('images/result_b.png'), 'B. お得に弱いタイプ',
        'セット商品などをみるとお得感を感じてなんでも手を伸ばしてしまうあなた。トイレットペーパーなどの生活必需品はともかく、スナック菓子などの必須ではないものを買いすぎていませんか？'),
    ResultModel(const AssetImage('images/result_c.png'), 'C. ごほうびタイプ',
        '「今月はあんなに頑張ったんだから…」と、不必要なものを買ったりしてしまいがち。自分を労ってあげたい時は自分自身に「よく頑張ったね」と語りかけるだけも癒されたりしますよ。'),
    ResultModel(const AssetImage('images/result_d.png'), 'D. 言い訳タイプ',
        '「先月はあまりお小遣いを使わなかったから」など、「〜だから」という理由をつけて無駄遣いしてしまうタイプ。本当に欲しいものは理由探しなどしないはずなので、理由を探している自分に気づいたら少し距離を置くと改善できるかも。'),
    ResultModel(const AssetImage('images/result_e.png'), 'E. 考えすぎタイプ',
